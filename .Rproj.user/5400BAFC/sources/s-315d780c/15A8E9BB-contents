...
for (i in Alle){
  ...
  png(file=paste("C:/Users/charl/Desktop/Bilder/Hist", names[j] , "MitErfahrung.png", sep=""),
    ...
    hist(i[i$ErfahrungBank == "Ja",]$values, breaks = breaks, main = paste("Histogramm mit Erfahrung von" , names[j]), xlab = names[j], ylab = "Häufigkeit")
    dev.off()
    png(file=paste("C:/Users/charl/Desktop/Bilder/Hist", names[j], "OhneErfahrung.png", sep=""),
        width=600, height=350)
    hist(i[i$ErfahrungBank == "Nein",]$values, breaks = breaks, main = paste("Histogramm ohne Erfahrung von" , names[j]), xlab = names[j], ylab = "Häufigkeit")
    dev.off()
}
...
plots[[6]] <- ggboxplot(AttraktivitaetBE, x = "ErfahrungBank", y = "values", add.params = list(color = "grey50", size = 1), xlab = " ", ylab = "Antworten Fragebogen", title = "Attraktivität") + theme( plot.title = element_text(hjust = 0.5), text = element_text(size = 18))
